...
#Remove data from 2018 that have been acquired later
final_CA50 <-  subset(final_merged,population%in%paste0("CA",1:3,"50") & year!="2018")
final_CA50$population <- as.factor(as.character(final_CA50$population))
table(final_CA50$population)

table(final_CA50[,c("year","population")])
vect_P_traits <- c("T12","T13","T21","T23","T31","T32")

# Scale covariates and trait values
for(j in c('temperature',"rel_humidity","logD")){
final_CA50[,j] <- (final_CA50[,j]-mean(final_CA50[,j]))/sd(final_CA50[,j])
}
...
for (i in sort(unique(final_CA50$population))) {
print(i)
	temp_final = subset(final_CA50,   population == i)
	...
	k=k+1
	VCV_mat_CA50[[k]]=list(Population = i, N_measurement = nrow(temp_final), G1_mat = matrix(post_dist[1:nb_trait^2], 
...
save(list=ls(),file='Output_files/RData/VCV_CA50.RData')
...
pdf(file='plots/G_mat_CA50.pdf',h=8,w=5.5)
par(mar=c(5,7,4,2))
vect_Var <- c(2:6,9:12,16:18,23,24,30,1,8,15,22,29,36)
vProb <- .95


plot(c(VCV_mat_CA50[[1]]$G1_mat)[vect_Var],c(24:10,6:1),yaxt="n",bty="n",xlim=c(-.25,.50),xlab="Genetic (co-)variances",xaxt="n",type='n',ylab="",cex.lab=1.2)
...
points(c(VCV_mat_CA50[[i]]$G1_mat)[vect_Var],c(24:10,6:1)+(.25*(i-1)),pch=21,bg="black",cex=.6)
}
dev.off()
