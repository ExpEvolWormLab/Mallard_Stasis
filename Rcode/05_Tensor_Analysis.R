...
df_for_tensor= rbind(final_A6140, final_CA50, final_CA100)
...
rm(i)
rm(VCV_mat_A6140,VCV_mat_CA50,VCV_mat_CA100,VCV_mat);gc()
...
save(list=c("ped_all","population_for_ped","n","m","Gnames","Garray","df_for_tensor","Earray2","traitnames","nb_trait"),file='Output_files/RData/File_for_parallel_processing.RData')

run_parallel_MCMC <- function(i){
  
	library(MCMCglmm)
	library(dae)
	load('Output_files/RData/File_for_parallel_processing.RData')
...
#save(list=ls(),file='Output_files/RData/Tensor_processed_SMALL.Rdata')
...
pdf('plots/EigenT_correlations_with_gamma.pdf',w=5)
par(mfrow=c(2,1))
for(k in c(1,3)){
