# This code projet all G matrix on the selection surface (gamma eigenvectors) and produce figure 5
...
rm(VCV_mat_A6140);rm(VCV_mat_CA50);rm(VCV_mat_CA100)

load('Output_files/RData/Analysis_Cemee_Pop_WI_with_gamma_with_Gas_only_Lisbon_with_pop_fixed.RData')


proj_M = eigen(gamma)$vectors
load("Output_files/RData/WI_G-matrix-all.RData")
rm(VCV_mat_A6140)
...
# We have 1.000 MCMC posteriors means, we will split them in 1 x 1000
list_pts=NULL
...
for(i in 1:ncol(post_mod_gamma_Rdm)){
temp_vect <-post_mod_gamma_Rdm[i,]
...
rdm_spl=NULL
for(i in 1:400) rdm_spl=rbind(rdm_spl ,sample(1:1000,10))
...
vect_col =c("black", "violetred1","darkorange","firebrick","forestgreen")
vect_y_pops=rep(c(0,100,-33),c(1,3,1))

vect_margin=rbind(c(1.5,5,3.5,0),c(1.5,2.5,3.5,2.5),c(1.5,0,3.5,5),
c(4,5,1,0),c(4,2.5,1,2.5),c(4,0,1,5))
adjust_x=c(0,rep(c(-5,0,5),1),0)
...
plot(post_modes_temp[c(1,2,4,6,8)]~vect_y_pops,ylim=c(0.002,0.3),type="n",log="y",xlim=c(-35,120),xlab="",ylab="",bty="n",xaxt="n",yaxt="n",main= vect_main[k],cex.main=2)
if(k==1) mtext(side=2, "Genetic variance",padj=-4,adj=-.6)
Ne_est=1000
lines(c(-33,0),(1-2/(Ne_est))^c(0,173)*(post_modes_temp[8]),lwd=2,lty=2,col="lightgreen")
...
vect80=t(matrix(unlist(tapply(temp_pop$Variance,temp_pop$pop,function(x){
	...
	arrows((vect_y_pops+adjust_x), vect80[c(1,5:8),1], (vect_y_pops+adjust_x), vect80[c(1,5:8),2],	code=0,length=0,angle=90,col= vect_col,lwd=3)
	arrows((vect_y_pops+adjust_x), vect90[c(1,5:8),1], (vect_y_pops+adjust_x), vect90[c(1,5:8),2],	code=3,length=.07,angle=90,col= grey(.6))

points(post_modes_temp[c(1,2,4,6,8)]~I(vect_y_pops+ adjust_x),pch=16,col= vect_col)

if(k==5) mtext(side=1, "Generation of focal evolution",padj=3)
if(k==4) legend(-10,.01,c("Wild isolates","A6140","CA[1-3]100"),lwd=3,col=c("forestgreen","black","orange"))
}
dev.off()
save(list=ls(),file="Output_files/RData/G_matrix_projection_for_plot_noCA50.RData")
