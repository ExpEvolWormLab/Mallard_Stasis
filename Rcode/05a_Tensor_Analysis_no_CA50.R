# This code computes the Tensor analysis comparing the G matrix of the A6140 and CA[1-3]100 populations only (removing CA[1-3]50)
...
df_for_tensor= rbind(final_A6140, final_CA100)
...
save(list=c("ped_all","population_for_ped","n","m","Gnames","Garray","df_for_tensor","Earray2","traitnames","nb_trait"),file='Output_files/RData/File_for_parallel_processing_noCA50.RData')
...
run_parallel_MCMC <- function(i){
	...
	library(dae)
  library(data.table)
	load('Output_files/RData/File_for_parallel_processing_noCA50.RData')
  ...
  a.pop <- cumsum(as.numeric(tapply(ped_all$id, population_for_ped$population, length)))
	...
	rand.model_MCMC.temp <- MCMCglmm(cbind(c(T12, T13, T21, T23, T31, T32)) ~  trait - 1, random = ~us(trait):pop_label ,
		 rcov = ~us(trait):units, 
		 family = rep("gaussian", nb_trait), data = ped_lines_current, prior = prior_mod, verbose = FALSE,nitt=150, burnin=50,thin=10)
...
save(list=ls(),file="Output_files/RData/Tensor_processed_noCA50.Rdata")



### End of the parallel thread
#### Load the data and produce the result plots
rm(list=ls())
gc()

source('Rcode/functions_tensor.R', chdir = TRUE)
load("Output_files/RData/Tensor_processed_noCA50.Rdata")
...
pdf("plots/Figure3A.pdf",w=3,h=5)
par(mfrow=c(1,1))

plot((1:nnonzero)-0.2,unique(MCMC.covtensor$tensor.summary[1:(n*nnonzero),1]),xlab="",ylab=expression(alpha),pch=16,cex=1,xaxt="n",frame.plot=F,xlim=c(0.5,3.5),ylim=c(0,max(HPD.eT.val)),type="n",las=1)
...
legend(1.3,.05,legend=c("observed","randomised"),lty=c(1,5),pch=c(16,8),cex=.9,bty="n")
dev.off()
...
pdf("plots/Figure3B.pdf",w=3,h=5)

par(mfrow=c(1,1))
ylim_vect=rbind(c(-1,0),c(-.4,.6))
for (k in 1){
plot(1:m,MCMC.covtensor$av.G.coord[,k,],ylab="",xlab="",pch=16,xaxt="n",frame.plot=F,xlim=c(0.5,m+.5),ylim=ylim_vect[k,],main = "",type="n",las=1)
  abline(h=0,lty=2)
...
pdf("plots/Figure3C.pdf",w=3,h=5)

par(mfrow=c(1,1))
par(mar=c(5,4,4,2))
plot(1:m,rowMeans(e11.proj),ylab=expression(lambda),xlab="",pch=16,cex = 1,xaxt="n",frame.plot=F,xlim=c(0,7),ylim=c(0,(max(HPD.e11))),las=2,type="n",las=1)
axis(1,at=1:m,labels=Gnames,las=2)
