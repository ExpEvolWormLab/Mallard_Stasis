...
data_WI$gen= (-33); data_WI$replicate = "WI"; data_WI$root = "WI"

data_populations_OF5 <- subset(data_populations_herm,population=="OF5")
data_populations_OF5 $gen= (-5); data_populations_OF5 $replicate = "OF5"; data_populations_OF5 $root = "OF5"

dim(data_populations_herm)
...
data_populations_herm$root = ifelse(substring(data_populations_herm$population,1,1)!="C", "A", "CA")
data_populations_herm$replicate = ifelse(data_populations_herm$root=="A", substring(data_populations_herm$population,1,2), substring(data_populations_herm$population,1,3))

vect_phenotypes= data_populations_herm[, "mean_still"]
vect_phenotypes= data_populations_herm[data_populations_herm$replicate!="A0", "mean_still"]

temp_mod1 <- lmer(vect_phenotypes ~ gen+(1|root)+(1|data_group_name),data=subset(data_populations_herm,replicate!="A0"))
temp_mod1_no_root <- lmer(vect_phenotypes ~ gen+(1|data_group_name),data=subset(data_populations_herm,replicate!="A0"))

vect_phenotypes= data_populations_herm[data_populations_herm$root=="CA", "mean_still"]
temp_mod1 <- lmer(vect_phenotypes ~ gen+(1| data_group_name),data=subset(data_populations_herm,root=="CA"))
temp_mod2 <- lmer(vect_phenotypes ~ (1| data_group_name),data=subset(data_populations_herm,root=="CA"))
anova(temp_mod1,temp_mod2)
summary(temp_mod1)



png("plots/Mean_Still_in_populations.png",h=3500,w=5600,res=600)
...
for(male_herm in c(1,2)){
  ...
  if( male_herm==1){
    par(mar=c(5,6,2,3))
    plot(1,1,type="n",xlim=c(-60,240),ylim= ylim_vect_2[1,],main= "",xaxt="n",bty="n",xlab="",ylab= vect_ylab,las=1,yaxt="s",cex.lab=1.3)
    ...
    data_populations <- subset(data_populations_herm,!substring(population,1,2)=="CM")
    data_populations <- rbind(data_populations_herm, data_WI,data_populations_OF5)
    CI_list <- CI_list_herm
  }else{
    points(1,1,type="n",xlim=c(-60,240),ylim= ylim_vect_2[1,],main= "",xaxt="n",bty="n",xlab="",ylab= "",las=1,yaxt="n",cex.lab=1.3)
    data_populations <- subset(data_populations_male,!substring(population,1,2)=="CM")
    CI_list <- CI_list_male
  }
