...
#load('Output_files/RData/Analysis_Cemee_Pop_WI_with_gamma_FINAL.RData')

proj_M = eigen(gamma)$vectors
#Gamma_model_MCMC <- model_MCMC_with_betas
#rm(model_MCMC);gc()
...
#for(i in 1:ncol(gamma_post_Rdm)){
#temp_vect <-gamma_post_Rdm[,i]
...
pdf("plots/G_matrix_projection_with_WI_pred.pdf")
...
vect_margin=rbind(c(1.5,4,3.5,0),c(1.5,2,3.5,2),c(1.5,0,3.5,4),
c(4,4,1,0),c(4,2,1,2),c(4,0,1,4))
...
plot(post_modes_temp[c(1:7,10)]~vect_y_pops,ylim=c(0.002,0.3),type="n",log="y",xlim=c(-35,120),xlab="",ylab="",bty="n",xaxt="n",yaxt="n",main= vect_main[k],cex.main=2)
...
#ylimit_polygon2= (1-2/(Ne_est))^c(-33,140,140,-33)*rep(vect80[8,],each=2)
#polygon(x=c(-33,0,0,-33),y= ylimit_polygon2,col=adjustcolor("lightgreen", alpha.f = 0.3),border=NA)
...
dev.off()
save(list=ls(),file="Output_files/RData/G_matrix_projection_for_plot.RData")
